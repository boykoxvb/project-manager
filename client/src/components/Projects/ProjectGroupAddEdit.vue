<template>
  <div class="project-group__addedit" :class="{ [`color-${changes?.color}`]: true }">
    <div class="group__name">
      <div class="group__color">
        <v-icon icon="mdi-palette" size="small"> </v-icon>
    ...
    <div class="tools">
      ...
      <div class="tools-button tools-close">
        <v-icon @click.stop="cancelChanges" icon="mdi-close-outline" size="small"></v-icon>
      </div>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const hasChanges = computed((): boolean => {
      return (
        !!(
          changes.color != props?.group?.color ||
          changes.name != props?.group?.name ||
          (!props.group && changes.color && changes.color)
        ) && !!changes.name
      )
    })
