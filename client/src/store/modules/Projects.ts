...
const Projects: Module<IProjectsState, IRootState> = {
  ...
  getters: {
    ...
    filteredProjects(state, getters) {
      ...
      filteredByName = state.filterState.nameSearch
        ? getters.allProjects.filter((project: Project) =>
            project.name.includes(state.filterState.nameSearch)
          )
        : getters.allProjects

      filteredByGroup = state.filterState.groupFilter
        ? filteredByName.filter(
            (project: Project) => project.group == state.filterState.groupFilter
          )
        : filteredByName

      filteredByState = state.filterState.stateFilter
        ? filteredByGroup.filter(
            (project: Project) => project.state == state.filterState.stateFilter
          )
        : filteredByGroup
  ...
  actions: {
    async fetchGroups({ commit }) {
      ...
      const projectGroups = groupsDto.map(
        (dto) => new ProjectGroup(undefined, undefined, undefined, dto)
      )
    ...
    async addTask({ commit }, project: Project) {
      ...
      const res = await ProjectsService.addTask(
        new TaskDto('', ''),
        ProjectDto.convertFromObject(project)
      )
    ...
    async projectChanged({ commit }, { project, changes }) {
      ...
      changes.deadline
        ? commit('changeProjectDeadline', { project: project, deadline: changes.deadline })
        : ''
      changes.name ? commit('changeProjectName', { project: project, name: changes.name }) : ''
      changes.group.uuid
        ? commit('changeProjectGroup', { project: project, group_id: changes.group.uuid })
        : ''
    },
    ...
    async editGroup({ commit }, { group, changes }) {
      try {
        const res = await ProjectsService.changeProjectGroup(
          group.uuid,
          new ProjectGroupDto(changes)
        )
    ...
    async addGroup({ commit }, { name, color }) {
      ...
      try {
        const res = await ProjectsService.addProjectGroup({
          uuid: undefined,
          name: name,
          color: color,
          projects: [],
  ...
  mutations: {
    ...
    editGroup(state, { group, name, color }) {
      const targetGroup = state.groups.find((gr) => {
        return gr == group
      })
      if (!targetGroup) {
    ...
    changeProjectGroup(state, { project, group_id }) {
      ...
      if (!targetGroup)
        throw new Error(
          `Группы с Id ${group_id} не существует во Vuex. Изменение группы проекта невозможно.`
        )
